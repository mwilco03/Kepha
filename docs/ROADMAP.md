...
| **Internet exposure safety system** | Any internet-exposed service (port forward, DDNS+NAT, split DNS) defaults into a DMZ zone — not LAN. The exposed service lives in an isolated network segment. Access from DMZ to LAN requires explicit jump host or VPN policy (default deny DMZ→LAN). User can override to LAN but gets multi-step confirmation + security checklist. Dashboard warning badge when any exposure active. Audit tagged `internet_exposure`. Workflow: (1) create port forward → system auto-creates DMZ zone if none exists → target placed in DMZ, (2) user wants to reach LAN from DMZ → must configure jump host or WireGuard tunnel, (3) suggest VPN as alternative at every step. | Large |
...
| **Split DNS** | Different answers for internal vs external queries (e.g. internal services resolve to LAN IPs) | Small |
| **DNSSEC validation** | Validate upstream responses via Unbound integration | Small |
